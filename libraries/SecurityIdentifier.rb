...
class SecurityIdentifier < Inspec.resource(1)
  ...
  def initialize(opts = {})
    ...
    unless opts.respond_to?(:keys)
      raise "Invalid security_identifier param '#{opts}'. Please pass a hash with these supported keys: #{supported_opt_keys}"
    end
    unless (opts.keys - supported_opt_keys).empty?
      raise "Unsupported security_identifier options '#{opts.keys - supported_opt_keys}'. Supported keys: #[supported_opt_keys]"
    end
    unless opts.keys && ((opts.keys & supported_opt_keys).length == 1)
      raise 'Specifying more than one of :user :group or :unspecified for security_identifier is not supported'
    end
